...
const { postTodoSchema, getTodosSchema, getTodoSchema, patchTodoSchema, editStatusTodoSchema, deleteTodoSchema } = require("./_schemas");
...
const editTodoStatus = require("./edit-todo-status");
 ...
 * @param {express.NextFunction} next
 */
const editStatusTodo = async (req, res, next) => {
  try {
    httpValidator({ params: req.params }, editStatusTodoSchema);

    const result = await editTodoStatus({ id: req.params.id, user: req.user.id });
